...
import { 
  ...
  Euro, HardHat, Briefcase, Zap, MapPin, Calculator, Wallet, Receipt
} from 'lucide-react';
...
export default function ChantierDetail() {
  ...
  const [chantier, setChantier] = useState<any>({
    ...
    // KPI Financiers (Prévisionnel)
    montant_marche: 0, budget_euro: 0, taux_horaire_moyen: 45, cpi: 0,
    ...
    try {
        ...
        if (results[1].status === 'fulfilled' && results[1].value.data) {
            ...
            setChantier({
                ...
                // Financier - Defaults
                montant_marche: c.montant_marche || 0, budget_euro: c.budget_euro || 0, taux_horaire_moyen: c.taux_horaire_moyen || 45, cpi: c.cpi || 0,
                ...
                // Arrays
                risques: c.risques || [], epi: c.epi || [],
                mesures_acqpa: c.mesures_acqpa || {}, client_email: c.client_email || '', client_telephone: c.client_telephone || ''
            });
  ...
  const addFourniture = async () => {
      ...
      const payload = { chantier_id: id, fourniture_ref_id: newFourniture.fourniture_ref_id, qte_prevue: newFourniture.qte_prevue || 0, qte_consommee: 0 };
      const { error } = await supabase.from('chantier_fournitures').insert([payload]);
      if(error) alert("Erreur: " + error.message); else { setNewFourniture({ fourniture_ref_id: '', qte_prevue: 0 }); setSupplySearch(""); fetchChantierData(); }
  };
  ...
  const coutMO_Prevu = chantier.heures_budget * chantier.taux_horaire_moyen;
  const budgetMO = chantier.budget_euro || 0;
  const percentBudgetMO = budgetMO > 0 ? Math.round((coutMO_Reel / budgetMO) * 100) : 0;
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6 print:max-w-none print:p-2">
        ...
        {activeTab === 'suivi' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6 print:space-y-4">
                ...
                {/* 1. GRILLE DE SAISIE RAPIDE (Input Réels) */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 print:grid-cols-4">
                    ...
                    <div className="bg-white p-5 rounded-[25px] shadow-sm border border-gray-100 relative overflow-hidden group print:border-gray-300">
                        ...
                        <div className="flex items-center gap-2 mt-2"><span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded text-[10px] font-black">{tauxMargeNette.toFixed(1)}%</span><span className="text-[10px] font-bold text-gray-400">Après CPI & Frais</span></div>
                    </div>
                    <div className="bg-white p-5 rounded-[25px] shadow-sm border border-gray-100 relative overflow-hidden group print:border-gray-300">
                        ...
                        <p className="text-[10px] font-bold text-gray-400 mt-1">Sur {coutMO_Prevu.toLocaleString()} € prévu</p>
                    </div>
                </div>

                {/* 3. GRAPHIQUES DE DELTA */}
                <div className="bg-white rounded-[30px] p-8 shadow-sm border border-gray-100 print:shadow-none print:border-gray-300 no-break">
                    ...
                    <div className="space-y-6">
                        <div className="space-y-1">
                            <div className="flex justify-between text-xs font-bold uppercase"><span className="text-gray-600 flex items-center gap-2"><Users size={14}/> Main d'Oeuvre</span><span className={coutMO_Reel > coutMO_Prevu ? 'text-red-500' : 'text-emerald-600'}>{coutMO_Reel.toLocaleString()}€ / {coutMO_Prevu.toLocaleString()}€</span></div>
                            <div className="w-full bg-gray-100 h-4 rounded-full overflow-hidden flex relative"><div className="absolute top-0 bottom-0 left-1/2 w-0.5 bg-black/10 z-10"></div><div className="h-full bg-blue-300" style={{width: '50%'}}></div><div className={`h-full ${coutMO_Reel > coutMO_Prevu ? 'bg-red-400' : 'bg-emerald-400'}`} style={{width: `${Math.min(50, (coutMO_Reel / Math.max(1, coutMO_Prevu)) * 50)}%`}}></div></div>
                        </div>
                        <div className="space-y-1">
        ...
        {/* --- ONGLET 2 : INFOS (AVEC SAISIE DES BUDGETS) --- */}
        {activeTab === 'infos' && (
