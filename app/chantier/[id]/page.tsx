...
export default function ChantierDetail() {
  ...
  const [chantier, setChantier] = useState<IChantier>({
    ...
    montant_marche: 0, taux_horaire_moyen: 24, cpi: 19,
    ...
    try {
        ...
        if (results[1].status === 'fulfilled' && results[1].value.data) {
            ...
            setChantier({
                ...
                numero_otp: c.numero_otp || '', responsable: c.responsable || '', chef_chantier_id: c.chef_chantier_id || '', equipe_ids: c.equipe_ids || [],
                montant_marche: c.montant_marche || 0, taux_horaire_moyen: c.taux_horaire_moyen || 24, cpi: c.cpi || 19,
                heures_budget: c.heures_budget || 0, heures_consommees: c.heures_consommees || 0,
  ...
  const handleAddFacture = async () => {
    ...
    const { error } = await supabase.from('chantier_facturation').insert([{ ...newFacture, chantier_id: id }]);
    if(!error) {
  ...
  const deleteFacture = async (fid: string) => {
      if(confirm('Supprimer cette ligne ?')) {
          await supabase.from('chantier_facturation').delete().eq('id', fid);
          fetchChantierData();
      }
  };
  ...
  const handleSectionPrint = (sectionId: string) => {
    ...
    style.innerHTML = `
      @media print {
        ...
        /* Reset positioning and heights so it rolls out fully on paper */
        #${sectionId} { position: absolute; left: 0; top: 0; width: 100%; margin:0; padding:0; box-shadow:none; height: auto !important; min-height: 0 !important; display: block !important;}
        .custom-scrollbar { overflow: visible !important; height: auto !important; max-height: none !important; display: block !important; flex: none !important;}
  ...
  const handleSave = async () => {
    const toSave = { 
        ...
        // CORRECTION DU BUG : Si la date est vide, on envoie null à PostgreSQL
  ...
  const toggleTask = async (task: any) => { 
      const newStatus = !task.done; const realHoursValue = newStatus ? (task.heures_reelles || task.objectif_heures) : 0; 
      const updatedSubtasks = (task.subtasks || []).map((st: any) => ({ ...st, done: newStatus, avancement: newStatus ? 100 : 0 })); 
  ...
  const deleteTask = async (taskId: string) => { if(confirm('Supprimer cette tâche et ses sous-tâches ?')) { await supabase.from('chantier_tasks').delete().eq('id', taskId); const updatedList = tasks.filter(t => t.id !== taskId); setTasks(updatedList); updateChantierTotalHours(updatedList); } };
  const saveTaskLabel = async (taskId: string, newLabel: string) => { const { error } = await supabase.from('chantier_tasks').update({ label: newLabel }).eq('id', taskId); if(!error) { setTasks(tasks.map(t => t.id === taskId ? { ...t, label: newLabel } : t)); setEditingTaskId(null); } };

  // Helper Date : Calcule "Du X au Y" en fonction des dates des sous-tâches
  const getTaskDateRange = (subtasks: any[]) => {
      if (!subtasks || subtasks.length === 0) return '';
      const dates = subtasks.map(st => new Date(st.date)).filter(d => !isNaN(d.getTime()));
      if (dates.length === 0) return '';
      const minDate = new Date(Math.min(...dates.map(d => d.getTime())));
      const maxDate = new Date(Math.max(...dates.map(d => d.getTime())));
      const format = (d: Date) => d.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' });
      if (minDate.getTime() === maxDate.getTime()) return `Le ${format(minDate)}`;
      return `Du ${format(minDate)} au ${format(maxDate)}`;
  };
  ...
  const addSubTask = async (parentId: string) => { 
      ...
      const updatedSubtasks = [...(parentTask.subtasks || []), { id: Date.now(), label: newSubTask.label, heures: parseFloat(newSubTask.heures.toString()) || 0, effectif: parseInt(newSubTask.effectif.toString()) || 1, date: newSubTask.date, done: false, avancement: 0 }]; 
      ...
      const newTasks = tasks.map(t => t.id === parentId ? { ...t, subtasks: updatedSubtasks, objectif_heures: newTotalHours } : t); setTasks(newTasks); setNewSubTask({ label: '', heures: 0, date: '', effectif: 1 }); setActiveParentTask(null); 
  };
  ...
  const toggleSubTask = async (parentId: string, subtaskId: number) => { 
      ...
      const updatedSubtasks = parentTask.subtasks.map((st: any) => {
          if(st.id === subtaskId) {
              ...
              return { ...st, done: newDone, avancement: newDone ? 100 : 0 };
          }
          return st;
      }); 
  ...
  const saveSubTaskFull = async (parentId: string, subTaskId: number) => {
      ...
      const parentTask = tasks.find(t => t.id === parentId);
      const updatedSubtasks = parentTask.subtasks.map((st: any) => 
          st.id === subTaskId ? { ...st, label: editingSubTaskData.label, date: editingSubTaskData.date, effectif: parseInt(editingSubTaskData.effectif)||1, heures: parseFloat(editingSubTaskData.heures)||0 } : st
      );
      const newTotalHours = updatedSubtasks.reduce((acc: number, curr: any) => acc + (parseFloat(curr.heures) || 0), 0);
      const { error } = await supabase.from('chantier_tasks').update({ subtasks: updatedSubtasks, objectif_heures: newTotalHours }).eq('id', parentId);
      if(!error) { 
  ...
  const updateSubTaskProgress = async (parentId: string, subTaskId: number, avancement: number) => {
      const safeVal = Math.min(100, Math.max(0, avancement || 0));
      const parentTask = tasks.find(t => t.id === parentId);
      const updatedSubtasks = parentTask.subtasks.map((st: any) => st.id === subTaskId ? { ...st, avancement: safeVal, done: safeVal === 100 ? true : false } : st);
      
      const { error } = await supabase.from('chantier_tasks').update({ subtasks: updatedSubtasks }).eq('id', parentId);
      if (!error) {
          setTasks(tasks.map(t => t.id === parentId ? { ...t, subtasks: updatedSubtasks } : t));
      }
  };

  // Logistique, Fournitures...
  const addFourniture = async () => { if(!newFourniture.fourniture_ref_id) return alert("Sélectionnez un produit"); const selectedItem = stockFournitures.find(s => s.id === newFourniture.fourniture_ref_id); const payload = { chantier_id: id, fourniture_ref_id: newFourniture.fourniture_ref_id, nom: selectedItem?.nom || 'Sans nom', unite: selectedItem?.unite || 'u', qte_prevue: newFourniture.qte_prevue || 0, qte_consommee: 0 }; const { error } = await supabase.from('chantier_fournitures').insert([payload]); if(error) alert("Erreur: " + error.message); else { setNewFourniture({ fourniture_ref_id: '', qte_prevue: 0 }); setSupplySearch(""); fetchChantierData(); } };
        ...
        {activeTab === 'infos' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4">
                <div className="space-y-6">
                    <div className="bg-white rounded-[30px] p-6 shadow-sm border border-gray-100 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="col-span-2">
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Numéro d'Affaire / OTP</label>
                                <div className="flex items-center bg-gray-50 rounded-xl px-2 mt-1">
                                    <Hash size={16} className="text-gray-400 mr-2"/>
                                    <input value={chantier.numero_otp || ''} onChange={e => setChantier({...chantier, numero_otp: e.target.value})} className="w-full bg-transparent p-3 font-bold outline-none" />
                                </div>
                            </div>
                            ...
                            <div className="col-span-2 grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        className="w-full bg-gray-50 p-3 rounded-xl font-bold outline-none border border-transparent focus:border-[#00b894] transition-colors cursor-pointer"
                                    ...
                                    >
                                        ...
                                        <option value="stand_by">En Stand-by</option>
                                        <option value="termine">Terminé</option>
                                    </select>
                                </div>
                                <div>
                                    {chantier.statut === 'probable' ? (
                                        <div>
                                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">% Obtention</label>
                                            <div className="flex items-center bg-gray-50 rounded-xl px-3 border border-transparent focus-within:border-[#00b894] transition-colors">
                                                <input type="number" min="0" max="100" className="w-full bg-transparent p-3 font-bold outline-none" value={chantier.taux_reussite || 0} onChange={e => setChantier({...chantier, taux_reussite: parseFloat(e.target.value) || 0})} />
                                ...
                                <div>
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Début Prévu</label>
                                    <div className="flex items-center bg-gray-50 rounded-xl px-3 border border-transparent focus-within:border-[#00b894] transition-colors">
                                        <Calendar size={14} className="text-gray-400 mr-2" />
                                        <input type="date" className="w-full bg-transparent p-3 font-bold outline-none" value={chantier.date_debut || ''} onChange={e => setChantier({...chantier, date_debut: e.target.value})} />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Fin Prévue</label>
                                    <div className="flex items-center bg-gray-50 rounded-xl px-3 border border-transparent focus-within:border-[#00b894] transition-colors">
                                        <Calendar size={14} className="text-gray-400 mr-2" />
                                        <input type="date" className="w-full bg-transparent p-3 font-bold outline-none" value={chantier.date_fin || ''} onChange={e => setChantier({...chantier, date_fin: e.target.value})} />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="col-span-2 p-4 bg-gray-50 rounded-2xl border border-gray-100 grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    ...
                                    <input value={chantier.client || ''} onChange={e => setChantier({...chantier, client: e.target.value})} className="w-full bg-white p-2 rounded-lg font-bold outline-none" />
                                </div>
                                <div className="col-span-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Adresse</label>
                                    <input value={chantier.adresse || ''} onChange={e => setChantier({...chantier, adresse: e.target.value})} className="w-full bg-white p-2 rounded-lg font-bold outline-none" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1"><Phone size={10}/> Tel</label>
                                    <input value={chantier.client_telephone || ''} onChange={e => setChantier({...chantier, client_telephone: e.target.value})} className="w-full bg-white p-2 rounded-lg font-bold outline-none" />
                                </div>
                                <div>
                                    ...
                                    <input value={chantier.client_email || ''} onChange={e => setChantier({...chantier, client_email: e.target.value})} className="w-full bg-white p-2 rounded-lg font-bold outline-none" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-[30px] p-6 shadow-sm border border-gray-100 space-y-4">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Chargé d'Affaire (Responsable)</label>
                            <div className="flex items-center bg-gray-50 rounded-xl px-2 mt-1">
                                <Briefcase size={16} className="text-gray-400 mr-2"/>
                                <input value={chantier.responsable || ''} onChange={e => setChantier({...chantier, responsable: e.target.value})} className="w-full bg-transparent p-3 font-bold outline-none" placeholder="Nom du responsable..." />
                            </div>
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Responsable Sur Site (Chef)</label>
                            <div className="flex items-center bg-gray-50 rounded-xl px-2 mt-1 border border-transparent focus-within:border-blue-300 transition-colors">
                                ...
                                <select className="w-full bg-transparent p-3 font-bold outline-none cursor-pointer" value={chantier.chef_chantier_id || ''} onChange={(e) => setChantier({...chantier, chef_chantier_id: e.target.value})}>
            ...
            <div className="space-y-6">
                <div className="bg-white rounded-[30px] p-6 shadow-sm border border-gray-100 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2 bg-blue-50 p-3 rounded-xl border border-blue-100">
                            <label className="text-[10px] font-bold text-blue-700 uppercase tracking-widest">Budget M.O. (Calculé)</label>
                            <div className="grid grid-cols-3 gap-2 mt-1">
                                <div><label className="text-[8px] uppercase">Heures</label><input type="number" value={chantier.heures_budget || 0} onChange={e => setChantier({...chantier, heures_budget: parseFloat(e.target.value) || 0})} className="w-full bg-white rounded p-1 font-bold text-center" placeholder="H"/></div>
                                <div><label className="text-[8px] uppercase">Taux</label><input type="number" value={chantier.taux_horaire_moyen} onChange={e => setChantier({...chantier, taux_horaire_moyen: parseFloat(e.target.value) || 0})} className="w-full bg-white rounded p-1 font-bold text-center" placeholder="€"/></div>
                                <div><label className="text-[8px] uppercase">CPI</label><input type="number" value={chantier.cpi} onChange={e => setChantier({...chantier, cpi: parseFloat(e.target.value) || 0})} className="w-full bg-white rounded p-1 font-bold text-center" placeholder="€"/></div>
                            </div>
                            <div className="text-right mt-2 font-black text-blue-800 text-lg">Total: {((chantier.heures_budget || 0) * (chantier.taux_horaire_moyen + chantier.cpi)).toLocaleString()} €</div>
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Matériaux Prévu</label>
                            <input type="number" value={chantier.cout_fournitures_prevu || 0} onChange={e => setChantier({...chantier, cout_fournitures_prevu: parseFloat(e.target.value) || 0})} className="w-full bg-gray-50 p-2 rounded-lg font-bold text-sm outline-none" />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Sous-Traitance Prévu</label>
                            <input type="number" value={chantier.cout_sous_traitance_prevu || 0} onChange={e => setChantier({...chantier, cout_sous_traitance_prevu: parseFloat(e.target.value) || 0})} className="w-full bg-gray-50 p-2 rounded-lg font-bold text-sm outline-none" />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Location Prévu</label>
                            <input type="number" value={chantier.cout_location_prevu || 0} onChange={e => setChantier({...chantier, cout_location_prevu: parseFloat(e.target.value) || 0})} className="w-full bg-gray-50 p-2 rounded-lg font-bold text-sm outline-none" />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Frais Généraux (Prévu)</label>
                            <input type="number" value={chantier.frais_generaux || 0} onChange={e => setChantier({...chantier, frais_generaux: parseFloat(e.target.value) || 0})} className="w-full bg-gray-50 p-2 rounded-lg font-bold text-sm outline-none" />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[30px] p-6 shadow-sm border border-gray-100 space-y-4">
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                        <div>
                            <h4 className="font-bold text-gray-800">Module ACQPA</h4>
                            <p className="text-xs text-gray-400">Activer le suivi qualité peinture</p>
                        </div>
        ...
        {activeTab === 'planning' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div id="tasks-view" className="bg-[#34495e] rounded-[30px] p-6 shadow-xl text-white relative flex flex-col min-h-[500px]">
                    ...
                    {/* Conteneur principal des tâches (scrollable à l'écran, déroulé à l'impression) */}
                                        ...
                                        <div className="flex-1">
                                            {editingTaskId === t.id ? (
                                                ...
                                                <div className="flex justify-between group">
                                                    <p className={`text-sm font-black uppercase tracking-tight ${t.done ? 'line-through text-gray-500' : 'text-[#ff9f43]'}`}>
                                                        {t.label}
                                                        {getTaskDateRange(t.subtasks) && <span className="text-[10px] font-bold text-gray-400 ml-2 normal-case border border-gray-500 px-1 rounded">({getTaskDateRange(t.subtasks)})</span>}
                                                    </p>
                                            ...
                                            {editingSubTaskData?.id === st.id ? (
                                                <div className="flex flex-wrap items-center gap-2 w-full bg-black/30 p-2 rounded mb-1 border border-white/10">
                                                    ...
                                                    <div className="flex gap-1 ml-auto">
                                                        <button onClick={(e) => { e.stopPropagation(); saveSubTaskFull(t.id, st.id); }} className="bg-green-500 px-2 py-1 rounded text-white text-[10px] font-bold hover:bg-green-600">OK</button>
                                                        <button onClick={(e) => { e.stopPropagation(); setEditingSubTaskData(null); }} className="bg-red-500 px-2 py-1 rounded text-white text-[10px] font-bold hover:bg-red-600">X</button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="flex items-center justify-between text-[11px] py-1 group">
                                                    <div className="flex items-center gap-2 cursor-pointer flex-1" onClick={() => toggleSubTask(t.id, st.id)}>
                                                        ...
                                                        <span className={`${st.done ? 'line-through text-white/30' : 'text-white/80'} hover:text-white flex items-center gap-2`}>
                                                            {st.label}
                                                            {st.date && <span className="text-[9px] text-[#00b894] font-bold">[{new Date(st.date).toLocaleDateString('fr-FR')}]</span>}
                                                        </span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[9px] font-bold text-white/40 hidden md:inline">{st.heures}h / {st.effectif}p</span>
                                                        <div className="flex items-center gap-1 bg-black/30 px-1 rounded border border-white/10" title="% Avancement">
                                                            <input type="number" min="0" max="100" className="bg-transparent w-8 text-[9px] font-bold text-white outline-none text-right" value={st.avancement || 0} onChange={(e) => updateSubTaskProgress(t.id, st.id, parseInt(e.target.value))} onClick={(e) => e.stopPropagation()}/>
                                                            <span className="text-[9px] text-white/50">%</span>
                                                        </div>
                                                        <div className="opacity-0 group-hover:opacity-100 flex gap-1 print:hidden">
                                                            <button onClick={(e) => { e.stopPropagation(); setEditingSubTaskData({id: st.id, label: st.label, date: st.date || '', heures: st.heures, effectif: st.effectif}); }} className="text-blue-400 hover:text-white"><Pencil size={10}/></button>
                ...
                {/* 2. GANTT VISUEL (Lecture Seule avec jauge d'avancement) */}
                        ...
                        {tasks.map((t, idx) => (
                            <div key={t.id} className="relative">
                                <div className="flex items-center gap-4 mb-2">
                                    <div className="w-48 font-bold text-sm truncate" title={t.label}>
                                        {t.label}
                                        {getTaskDateRange(t.subtasks) && <div className="text-[9px] text-gray-400 mt-0.5">{getTaskDateRange(t.subtasks)}</div>}
                                    </div>
                                ...
                                {t.subtasks && t.subtasks.map((st: any) => {
                                    const leftPos = (new Date(st.date || new Date()).getDate() % 30) * 3;
                                    const targetWidth = Math.max(15, st.heures * 2);
                                    const safeWidth = Math.min(targetWidth, 100 - leftPos);
                                    const isDone = st.done || st.avancement === 100;
                                    ...
                                    return (
                                        <div key={st.id} className="flex items-center gap-4 mb-1 pl-8 relative group">
                                            ...
                                            <div className="flex-1 bg-gray-50 h-6 rounded-full relative overflow-hidden border border-gray-100">
                                                <div 
                                                    className={`absolute top-0 h-full rounded-full overflow-hidden flex items-center justify-center px-2 text-[9px] text-white font-bold ${isDone ? 'bg-blue-400' : 'bg-orange-300'}`} 
                                                    style={{ left: `${leftPos}%`, width: `${safeWidth}%` }}
                                                >
