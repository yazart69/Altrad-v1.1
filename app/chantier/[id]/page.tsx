...
function DashboardTab({ data }: { data: any }) {
  ...
  const { pieStyle, chartData } = useMemo(() => {
    ...
    const totalChart = Math.max(finances.montantMarcheActif, d.reduce((acc, curr) => acc + curr.value, 0));
    let currentAngle = 0;
    const gradientParts = d.map(item => { 
...
function FournituresTab({ data }: { data: any }) {
  ...
  const updateConsommation = async (fId: string, val: number) => { const newVal = Math.max(0, val); setFournituresPrevu((prev: any[]) => prev.map(f => f.id === fId ? { ...f, qte_consommee: newVal } : f)); await supabase.from('chantier_fournitures').update({ qte_consommee: newVal }).eq('id', fId); };
...
function PlanningTab({ data }: { data: any }) {
  ...
  const getTaskDateRange = (subtasks: any[]) => {
      ...
      const minDate = new Date(Math.min(...dates.map(d => d.getTime()))); const maxDate = new Date(Math.max(...dates.map(d => d.getTime())));
  ...
  const deleteTask = async (taskId: string) => { if(confirm('Supprimer cette tâche et ses sous-tâches ?')) { await supabase.from('chantier_tasks').delete().eq('id', taskId); const updatedList = tasks.filter(t => t.id !== taskId); setTasks(updatedList); actions.updateChantierTotalHours(updatedList); } };
