...
import { 
  ...
  Plus, Minus, Users, Percent, Truck, Package, Wrench, Mail, Phone, BarChart2, CornerDownRight, 
  UserPlus, Search, TrendingUp, TrendingDown, UserCheck, Scale, Printer, PieChart, Target, 
  Euro, HardHat, Briefcase, Calculator, Wallet, Receipt, Hash, Check, LayoutDashboard,
  CreditCard, FileCheck, CalendarClock, AlertCircle
} from 'lucide-react';
...
export default function ChantierDetail() {
  ...
  const updateFactureStatut = async (fid: string, newStatut: string) => {
      ...
      fetchChantierData(); // Refresh pour recalculer les totaux
  };
  ...
  // Helpers standards (identiques aux versions précédentes)
  ...
  // On filtre les TS / PlusValues qui sont "validés" (ou on prend tout si on veut le potentiel)
  const totalTS = facturationList
    .filter(f => (f.type === 'TS' || f.type === 'PlusValue') && f.statut === 'valide')
    .reduce((acc, f) => acc + (f.montant || 0), 0);
  const totalMoinsValues = facturationList
    .filter(f => f.type === 'MoinsValue' && f.statut === 'valide')
    .reduce((acc, f) => acc + (f.montant || 0), 0);
  ...
  const totalFacture = facturationList
    .filter(f => (f.type === 'Facture' || f.type === 'Acompte' || f.type === 'Solde') && f.statut !== 'prevu')
    .reduce((acc, f) => acc + (f.montant || 0), 0);
    
  const totalEncaisse = facturationList
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6 print:space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:grid-cols-2">
                    ...
                    <div className="bg-[#f8f9fa] border border-dashed border-gray-300 p-6 rounded-[30px] print:hidden">
                        <h4 className="text-sm font-black uppercase text-gray-500 mb-4 flex items-center gap-2"><Calculator size={16}/> Saisie Coûts Réels</h4>
                ...
                <div className="bg-white rounded-[30px] p-8 shadow-sm border border-gray-100 print:shadow-none print:border-gray-300 no-break">
                    ...
                    <div className="w-full bg-gray-100 h-6 rounded-full overflow-hidden flex relative mb-6 border border-gray-200">
                         <div className="h-full bg-green-400 flex items-center justify-center text-[10px] font-black text-white" style={{width: `${(totalEncaisse/montantMarcheActif)*100}%`}}>Enc.</div>
                         <div className="h-full bg-blue-400 flex items-center justify-center text-[10px] font-black text-white" style={{width: `${((totalFacture - totalEncaisse)/montantMarcheActif)*100}%`}}>Fact.</div>
                    </div>
                    ...
                    <table className="w-full text-left text-xs mb-4">
                        ...
                        <tbody className="divide-y divide-gray-50 font-bold text-gray-700">
                            {facturationList.map(f => (
                                <tr key={f.id} className={f.type === 'TS' ? 'bg-purple-50/50' : ''}>
                                    ...
                                    <td className="p-3 text-gray-500">{f.date_echeance}</td>
                                    <td className="p-3 text-center"><select value={f.statut} onChange={(e) => updateFactureStatut(f.id, e.target.value)} className="bg-transparent outline-none cursor-pointer"><option value="prevu">Prévu</option><option value="envoye">Envoyé</option><option value="paye">Payé</option><option value="valide">Validé (TS)</option></select></td>
                                    <td className="p-3 text-right">{f.montant.toLocaleString()} €</td>
                    ...
                    <div className="flex gap-2 items-center bg-gray-50 p-2 rounded-xl print:hidden">
                        ...
                        <input type="date" className="bg-white p-2 rounded-lg text-xs outline-none border border-gray-200" value={newFacture.date_echeance} onChange={e => setNewFacture({...newFacture, date_echeance: e.target.value})} />
