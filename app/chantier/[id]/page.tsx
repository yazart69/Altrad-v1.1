...
export default function ChantierDetail() {
  ...
  const handleSectionPrint = (sectionId: string) => {
    ...
    style.innerHTML = `
      @media print {
        ...
        #${sectionId} { position: absolute; left: 0; top: 0; width: 100%; margin:0; padding:0; box-shadow:none; }
  ...
  const saveSubTaskLabel = async (parentId: string, subTaskId: number, newLabel: string) => {
      ...
      if(!error) { const newTasks = tasks.map(t => t.id === parentId ? { ...t, subtasks: updatedSubtasks } : t); setTasks(newTasks); setEditingSubTaskId(null); }
  };
                                    ...
                                    {t.subtasks && t.subtasks.map((st: any) => (
                                        <div key={st.id} className="flex items-center justify-between text-[11px] py-1 group">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="text-[9px] font-bold text-white/40 hidden md:inline">{st.heures}h</span>
                            ...
                            <div key={t.id} className="relative">
                                <div className="flex items-center gap-4 mb-2">
                                    ...
                                    <div className="flex-1 bg-gray-100 h-8 rounded-full relative overflow-hidden">
                                        <div className="absolute top-0 left-0 h-full bg-[#00b894] opacity-80 flex items-center px-3 text-[10px] text-white font-bold" style={{ width: `${Math.min(100, Math.max(10, t.objectif_heures * 2))}%` }}>{t.objectif_heures}h</div>
                                    </div>
                                </div>
                                {t.subtasks && t.subtasks.map((st: any) => {
