...
export default function ChantierDetail() {
  ...
  const [chantier, setChantier] = useState<any>({
    ...
    // KPI Financiers
    montant_marche: 0, // Prix de Vente
    taux_horaire_moyen: 24, // Taux de base
    cpi: 19, // Coût Production Indirect
    
    // Budgets & Réels (Hors MO)
    ...
    try {
        ...
        if (results[1].status === 'fulfilled' && results[1].value.data) {
            ...
            setChantier({
                ...c,
                // Infos
                client: c.client || '', adresse: c.adresse || '', client_email: c.client_email || '', client_telephone: c.client_telephone || '',
                ...
                taux_horaire_moyen: c.taux_horaire_moyen || 24, // Taux de base par défaut
                cpi: c.cpi || 19, // CPI par défaut
  ...
  const handleSave = async () => {
    const toSave = { 
        ...
        chef_chantier_id: chantier.chef_chantier_id || null, // Gestion UUID null
        // On sauvegarde toutes les données financières saisies
    ...
    if (error) alert("Erreur : " + error.message); else { alert('✅ Chantier sauvegardé !'); fetchChantierData(); }
  };

  // --- CALCULS FINANCIERS (MOTEUR) ---
  ...
  const margeBrute = montantMarche - coutDirect_Reel; // Prix Vente - Coûts Directs
  const tauxMargeBrute = montantMarche > 0 ? (margeBrute / montantMarche) * 100 : 0;
  
  // Marge Nette (On soustrait les Frais Généraux en plus)
  ...
  // --- PREPARATION DONNEES CAMEMBERT ---
  // Calculs pour le graphique (Percentages)
  const chartData = [
    { label: 'Main d\'Oeuvre', value: coutMO_Reel, color: '#3b82f6' }, // Blue
    { label: 'Matériaux', value: chantier.cout_fournitures_reel, color: '#f97316' }, // Orange
    { label: 'Sous-Traitance', value: chantier.cout_sous_traitance_reel, color: '#a855f7' }, // Purple
    { label: 'Location', value: chantier.cout_location_reel, color: '#ec4899' }, // Pink
    { label: 'Frais Généraux', value: chantier.frais_generaux_reel, color: '#64748b' }, // Gray
    { label: 'Marge Nette', value: Math.max(0, margeNette), color: '#10b981' } // Green
  ];
  
  // Total pour le graphique (Si < Montant Marché, le reste est la marge, sinon on dépasse)
  const totalChart = Math.max(montantMarche, chartData.reduce((acc, d) => acc + d.value, 0));
  
  // Génération du Conic Gradient pour le Camembert CSS
  let currentAngle = 0;
  ...
  // Si le total des coûts dépasse le marché, on comble avec du rouge (perte)
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6 print:max-w-none print:p-2">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar print:hidden">
            {[
                ...
                { id: 'infos', label: 'Infos Chantier', icon: FileText, color: 'bg-[#34495e]' }, // RENOMMÉ ICI
        ...
        {activeTab === 'suivi' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6 print:space-y-4">
                ...
                <div className="flex justify-between items-end border-b border-gray-200 pb-4 mb-6">
                    <div>
                        <h2 className="text-3xl font-black uppercase text-[#2d3436] tracking-tight">Rapport Financier</h2>
                        <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">{chantier.nom} — {chantier.client}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 print:grid-cols-4">
                    ...
                    <div className="bg-white p-5 rounded-[25px] shadow-sm border border-gray-100 relative overflow-hidden group print:border-gray-300">
                        ...
                        <div className="flex items-center gap-2 mt-2"><span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded text-[10px] font-black">{tauxMargeNette.toFixed(1)}%</span><span className="text-[10px] font-bold text-gray-400">Après Frais Gén.</span></div>
                    </div>
                    <div className="bg-white p-5 rounded-[25px] shadow-sm border border-gray-100 relative overflow-hidden group print:border-gray-300">
                        <div className="absolute top-0 right-0 p-4 opacity-10 text-orange-500"><HardHat size={50}/></div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Coût M.O. Réel</p>
                        <div className="text-3xl font-black text-orange-500 mt-1">{coutMO_Reel.toLocaleString()} €</div>
                        <p className="text-[10px] font-bold text-gray-400 mt-1">Taux chargé: {TAUX_CHARGE}€/h</p>
                    </div>
                </div>

                {/* GRAPHIQUE CAMEMBERT (NOUVEAU) & SAISIE RAPIDE */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:grid-cols-2">
                    ...
                    <div className="bg-white rounded-[30px] p-8 shadow-sm border border-gray-100 print:shadow-none print:border-gray-300 no-break flex flex-col items-center">
                        <h3 className="font-black uppercase text-gray-700 mb-6 flex items-center gap-2 self-start"><PieChart className="text-[#6c5ce7]"/> Décomposition du Prix de Vente</h3>
                        <div className="flex items-center gap-8 w-full justify-center">
                            {/* Le Graphique CSS */}
                            <div className="relative w-48 h-48 rounded-full border-4 border-white shadow-xl" style={pieStyle}>
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <div className="bg-white w-24 h-24 rounded-full flex items-center justify-center shadow-inner">
                                        <span className="font-black text-xs text-gray-400">Total Vente</span>
                                    </div>
                                </div>
                            </div>
                            {/* La Légende */}
                            <div className="space-y-2 text-xs">
                                {chartData.map((d, i) => (
                                    <div key={i} className="flex items-center gap-2">
                                        ...
                                        <span className="font-black text-gray-800">{Math.round((d.value/montantMarche)*100)}%</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {/* SAISIE RAPIDE (Cachée à l'impression) */}
                    <div className="bg-[#f8f9fa] border border-dashed border-gray-300 p-6 rounded-[30px] print:hidden">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase">Taux Horaire + CIP (Coût Prod. Indirect)</label>
                                <div className="flex gap-2">
                                    <div className="flex-1 relative"><Euro size={14} className="absolute left-3 top-3 text-gray-400"/><input type="number" className="w-full bg-white pl-8 p-3 rounded-xl text-sm font-bold border border-gray-200 outline-none" value={chantier.taux_horaire_moyen} onChange={e => setChantier({...chantier, taux_horaire_moyen: parseFloat(e.target.value)||0})} placeholder="Taux" /></div>
                                    <div className="flex-1 relative"><AlertOctagon size={14} className="absolute left-3 top-3 text-gray-400"/><input type="number" className="w-full bg-white pl-8 p-3 rounded-xl text-sm font-bold border border-gray-200 outline-none" value={chantier.cpi} onChange={e => setChantier({...chantier, cpi: parseFloat(e.target.value)||0})} placeholder="CPI" /></div>
                                </div>
                                <p className="text-[10px] text-blue-500 font-bold mt-1 text-right">Taux Chargé total = {TAUX_CHARGE} €/h</p>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-[9px] font-bold text-gray-400 uppercase">Matériaux Réel</label><input type="number" className="w-full bg-white p-3 rounded-xl text-sm font-bold border border-gray-200 outline-none" value={chantier.cout_fournitures_reel} onChange={e => setChantier({...chantier, cout_fournitures_reel: parseFloat(e.target.value)||0})} /></div>
                                <div><label className="text-[9px] font-bold text-gray-400 uppercase">Sous-Traitance Réel</label><input type="number" className="w-full bg-white p-3 rounded-xl text-sm font-bold border border-gray-200 outline-none" value={chantier.cout_sous_traitance_reel} onChange={e => setChantier({...chantier, cout_sous_traitance_reel: parseFloat(e.target.value)||0})} /></div>
                                <div><label className="text-[9px] font-bold text-gray-400 uppercase">Location Réel</label><input type="number" className="w-full bg-white p-3 rounded-xl text-sm font-bold border border-gray-200 outline-none" value={chantier.cout_location_reel} onChange={e => setChantier({...chantier, cout_location_reel: parseFloat(e.target.value)||0})} /></div>
                                <div><label className="text-[9px] font-bold text-gray-400 uppercase">Frais Généraux Réel</label><input type="number" className="w-full bg-white p-3 rounded-xl text-sm font-bold border border-gray-200 outline-none" value={chantier.frais_generaux_reel} onChange={e => setChantier({...chantier, frais_generaux_reel: parseFloat(e.target.value)||0})} /></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        {/* --- ONGLET INFOS CHANTIER (RENOMMÉ & COMPLÉTÉ) --- */}
        {activeTab === 'infos' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4">
                
                {/* BLOC 1 : IDENTIFICATION & RH */}
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-[30px] p-6 shadow-sm border border-gray-100 space-y-4">
                        ...
                        <div className="space-y-4">
                            <div><label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Conducteur de Travaux (Responsable)</label><div className="flex items-center bg-gray-50 rounded-xl px-2 mt-1"><Briefcase size={16} className="text-gray-400 mr-2"/><input value={chantier.responsable || ''} onChange={e => setChantier({...chantier, responsable: e.target.value})} className="w-full bg-transparent p-3 font-bold outline-none" placeholder="Nom du responsable..." /></div></div>
                    ...
                    <div className="bg-white rounded-[30px] p-6 shadow-sm border border-gray-100 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="col-span-2 bg-blue-50 p-3 rounded-xl border border-blue-100">
                                ...
                                <div className="text-right mt-2 font-black text-blue-800 text-lg">Total: {coutMO_Prevu.toLocaleString()} €</div>
                            </div>
        ...
        {/* --- AUTRES ONGLETS (LOGISTIQUE, FOURNITURES, PLANNING, ETC.) --- */}
