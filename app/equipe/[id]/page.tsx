...
export default function FicheEmploye() {
  ...
  // CORRECTION TECHNIQUE : On retire l'ID du payload pour éviter l'erreur 400
  const handleSave = async () => {
    if (!emp) return;
    setIsSaving(true);
    
    // On crée une copie sans l'id car Supabase interdit de modifier la clé primaire
    const { id: _, ...payload } = emp;

    const { error } = await supabase.from('employes').update(payload).eq('id', id);
    setIsSaving(false);
    
    if (!error) alert("✅ Dossier mis à jour avec succès !");
    else alert("❌ Erreur lors de la sauvegarde (Vérifiez les colonnes).");
  };
  ...
  return (
    <div className="p-4 md:p-10 font-['Fredoka'] max-w-6xl mx-auto pb-32 bg-gray-50 min-h-screen">
      
      {/* HEADER ACTIONS */}
      <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-400 font-bold hover:text-black transition-all uppercase text-xs">
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div className="bg-[#fff8e1] p-8 rounded-[40px] text-yellow-900 shadow-sm border border-yellow-100 relative">
            ...
            <textarea 
                ...
                placeholder="Notez ici les infos importantes..." 
        ...
        {/* COLONNE DROITE : FORMATIONS & DOCUMENTS */}
        <div className="col-span-12 lg:col-span-8 space-y-6">
            <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm">
                <div className="flex justify-between items-start mb-8">
                    <div>
                        ...
                        <p className="text-xs text-gray-400 font-bold mt-1">Suivi des compétences</p>
                    </div>
                    <div className="flex gap-2">
                        <input type="text" placeholder="Ajouter..." className="bg-gray-50 border-none rounded-xl px-4 py-2 text-xs font-bold w-40 md:w-60" value={newForma} onChange={(e)=>setNewForma(e.target.value)} onKeyDown={(e)=>e.key === 'Enter' && addItem('forma')}/>
                            ...
                            <div className="flex flex-col md:flex-row justify-between gap-4 items-start md:items-center">
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="flex flex-col">
                                        ...
                                        <input type="date" value={f.exp} onChange={(e)=>{const c=[...emp.formations_json]; c[idx].exp=e.target.value; setEmp({...emp, formations_json:c})}} className="bg-transparent text-xs font-bold text-gray-600 border-none p-0"/>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <label className="cursor-pointer p-2 rounded-lg hover:bg-gray-200 relative">
                                        ...
                                        <button onClick={()=>{const c=emp.formations_json.filter((_:any,i:number)=>i!==idx); setEmp({...emp, formations_json:c})}} className="p-2 text-gray-300 hover:text-red-500 rounded-lg"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="bg-[#FFF9C4] p-1 rounded-[40px] shadow-sm border border-yellow-200 relative overflow-hidden">
                <div className="bg-[#FFF176] p-4 rounded-t-[35px] flex justify-between items-center px-8 border-b border-yellow-300/50">
                    ...
                    <div className="flex gap-2">
                        <input type="text" placeholder="Ajouter un document..." className="bg-white/50 border-none rounded-lg px-3 py-1.5 text-xs font-bold w-40" value={newDoc} onChange={(e)=>setNewDoc(e.target.value)} onKeyDown={(e)=>e.key === 'Enter' && addItem('doc')}/>
