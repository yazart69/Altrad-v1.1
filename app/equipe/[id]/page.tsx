...
export default function FicheEmploye() {
  ...
  const handleSave = async () => {
    ...
    if (!error) alert("Dossier et Statut mis à jour !");
  };
    ...
    <div className="p-4 md:p-10 font-['Fredoka'] max-w-6xl mx-auto pb-32">
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <div className="mt-8 pt-8 border-t border-gray-50">
              ...
              <input 
                type="text" 
                placeholder="Commentaire (ex: retour le 15/02)..."
            ...
            <div className="mt-6 pt-6 border-t border-gray-50 space-y-3">
              <label className="block text-[10px] font-black uppercase text-gray-400 tracking-tighter">Véhicule</label>
          ...
          <div className="bg-[#2d3436] p-8 rounded-[40px] text-white shadow-xl">
            <div className="flex items-center gap-2 mb-4 text-[#ff9f43] font-black uppercase text-xs tracking-widest"><Bell size={18} /> Convocations</div>
            <textarea value={emp.convocations_notes || ''} onChange={(e)=>setEmp({...emp, convocations_notes: e.target.value})} placeholder="Notes bureau / santé..." className="w-full bg-white/10 border-none rounded-2xl p-4 text-xs h-40 focus:ring-1 focus:ring-orange-500 placeholder:text-white/20"/>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          ...
          <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <input type="text" placeholder="Ajouter (ex: H0B0)..." className="bg-gray-50 border-none rounded-xl px-4 py-2 text-xs font-bold w-48" value={newHabil} onChange={(e)=>setNewHabil(e.target.value)} onKeyDown={(e)=>e.key === 'Enter' && addItem('habil')}/>
              ...
              {emp.habilitations_json.map((h:any, idx:number)=>(
                <div key={idx} className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl group transition-all hover:bg-gray-100">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    {h.doc_url && <button onClick={()=>setPreviewUrl(h.doc_url)} className="text-blue-500"><Eye size={18}/></button>}
                    <button onClick={()=>{const c = emp.habilitations_json.filter((_:any,i:number)=>i!==idx); setEmp({...emp, habilitations_json:c});}} className="text-gray-200 hover:text-red-500"><Trash2 size={16}/></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <input type="text" placeholder="Ajouter..." className="bg-gray-50 border-none rounded-xl px-4 py-2 text-xs font-bold w-48" value={newForma} onChange={(e)=>setNewForma(e.target.value)} onKeyDown={(e)=>e.key === 'Enter' && addItem('forma')}/>
            ...
            <div className="space-y-3">
              {emp.formations_json.map((f:any, idx:number)=>(
                <div key={idx} className="grid grid-cols-12 gap-4 items-center bg-gray-50 p-4 rounded-2xl group transition-all hover:bg-gray-100">
                  <div className="col-span-5 font-bold text-sm uppercase">{f.label}</div>
                  <div className="col-span-4 flex flex-col"><span className="text-[8px] font-black text-gray-400 uppercase">Validité</span><input type="date" value={f.exp} onChange={(e)=>{const c = [...emp.formations_json]; c[idx].exp = e.target.value; setEmp({...emp, formations_json:c});}} className="bg-transparent border-none text-[10px] font-black p-0 uppercase" /></div>
                  <div className="col-span-3 flex justify-end gap-3 items-center">
                    ...
                    <button onClick={()=>{const c = emp.formations_json.filter((_:any,i:number)=>i!==idx); setEmp({...emp, formations_json:c});}} className="text-gray-200 hover:text-red-500"><Trash2 size={16}/></button>
                  </div>
                </div>
              ))}
            </div>
              ...
              {emp.documents_divers_json.map((d:any, idx:number)=>(
                <div key={idx} className="flex items-center justify-between bg-gray-50 p-4 rounded-2xl group border border-transparent hover:border-purple-100 transition-all">
                  ...
                  <div className="flex items-center gap-2">
                    <label className="cursor-pointer"><input type="file" className="hidden" onChange={(e)=>handleUpload(e, 'doc', idx)} /><FileUp size={18} className={d.doc_url ? "text-green-500" : "text-gray-300"} /></label>
      ...
      {previewUrl && (
        <div className="fixed inset-0 bg-black/95 z-[200] flex flex-col items-center justify-center p-4">
          <button onClick={()=>setPreviewUrl(null)} className="absolute top-6 right-6 text-white p-3 bg-white/10 rounded-full hover:bg-white/20"><X size={32}/></button>
          <div className="max-w-4xl w-full h-[80vh] bg-white rounded-3xl overflow-hidden shadow-2xl relative">
             <iframe src={previewUrl} className="w-full h-full border-none" title="Preview" />
          </div>
          <div className="mt-6">
            <button onClick={()=>window.open(previewUrl, '_blank')} className="bg-white text-black px-10 py-4 rounded-2xl font-black uppercase text-xs flex items-center gap-2 shadow-xl shadow-white/10"><ExternalLink size={18}/> Ouvrir en plein écran</button>
          </div>
        </div>
      )}
    </div>
